...
\begin{lemma}
Let $h:[0,\infty) \to \R$ be a twice-differentiable function that satisfies $\frac{h'(x)}{h''(x)} \ge x$ for all $x \ge 0$.
Then,
$$
...
$$
With this notation, the left hand is
$$
...
$$
If we consider $x=\sqrt{\|u\|^2 + \|v\|^2 + 2 c \|u\| \cdot \|v\|}$, the assumption $\frac{h'(x)}{h''(x)} \ge x$
...
\end{align*}
Let's view the last expression as function of $g_t$. That is, let for any $u \in \H$,
$$
\Phi(u) = (1 + \langle u, \beta_T \rangle) \Wealth_{T-1} \ - \ \frac{2^T \cdot \Gamma \left(\frac{T+1}{2} + \frac{1}{2}\left\|u + \sum_{t=1}^{T-t} g_t \right\| \right) \cdot \Gamma \left(\frac{T+1}{2} - \frac{1}{2} \left\|u + \sum_{t=1}^{T-1} g_t \right\| \right)}{\pi \cdot T!} \; .
$$
We claim that $\Phi(u)$ is concave. This key insight is that $\Gamma(\cdot)$ is logaritmically convex. In more detail,
\begin{itemize}
\item $\log(\Gamma(\cdot))$ is convex and increasing on $(0, \infty)$; see Bohr-Mollerup theorem.
\item $u \mapsto \frac{T+1}{2} + \frac{1}{2}\left\|u + \sum_{t=1}^{T-t} g_t \right\|$ and $u \mapsto \frac{T+1}{2} - \frac{1}{2} \left\|u + \sum_{t=1}^{T-1} g_t \right\|$ are convex on $\H$.
\item $u \mapsto \log \Gamma \left(\frac{T+1}{2} + \frac{1}{2}\left\|u + \sum_{t=1}^{T-t} g_t \right\| \right) + \log \Gamma\left( \frac{T+1}{2} - \frac{1}{2} \left\|u + \sum_{t=1}^{T-1} g_t \right\|\right)$
is convex by composition and summation.
\item $\exp(\cdot)$ is convex and increasing on $(-\infty, \infty)$
\item $u \mapsto \exp\left\{\log \Gamma \left(\frac{T+1}{2} + \frac{1}{2}\left\|u + \sum_{t=1}^{T-t} g_t \right\| \right) + \log \Gamma\left( \frac{T+1}{2} - \frac{1}{2} \left\|u + \sum_{t=1}^{T-1} g_t \right\|\right) \right\}$
is convex by composition
\item $u \mapsto \Gamma \left(\frac{T+1}{2} + \frac{1}{2}\left\|u + \sum_{t=1}^{T-t} g_t \right\| \right) \cdot \Gamma\left( \frac{T+1}{2} - \frac{1}{2} \left\|u + \sum_{t=1}^{T-1} g_t \right\|\right)$
is convex
\end{itemize}
Thus, we see that $\Phi(u)$ is concave.

We now claim that
$$
\forall u \in \H \qquad \qquad \Phi(u) \ge \min_{s \in \{+1,-1\}} \Phi\left( r \|u\| \cdot \|\beta_T\| \right) \; .
$$
To see that note that we can write $\Phi(u)$ as
$$
\Phi(u) = a \langle u, \beta_T \rangle + h(\|u + b \beta_T\|) + c
$$
where $a,b,c$ are positive constants; this representation follows from that $\beta_T = \frac{1}{T}\sum_{t=1}^{T-1} g_t$.
