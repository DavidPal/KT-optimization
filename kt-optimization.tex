...
Let $h:[0,\infty) \to \R$ be a twice-differentiable function that satisfies $x \cdot h''(x) \le h'(x)$ for all $x \ge 0$.
Then,
$$
...
\begin{lemma}
Let $\psi^{(m)}(z) = \frac{d^{m+1}}{dz^{m+1}} \ln(\Gamma(z))$ be the polygamma functions, $m \ge 0$.
Then, for any $0 \le z < a$,
$$
z \left( \psi^{(0)}\left(a + z \right) - \psi^{(0)}\left( a - z \right) \right)^2
+ z \left( \psi^{(1)}\left(a + z \right) + \psi^{(1)}\left(a - z \right) \right)
\ge \psi^{(0)}\left(a + z \right) - \psi^{(0)}\left(a - z \right) \; .
$$
\end{lemma}
...
\end{align*}
We now apply the Lemma~\ref{lemma:extremes} with
$$
h(x) = - C \cdot \Gamma \left(\frac{T+1}{2} + \frac{x}{2} \right) \cdot \Gamma \left(\frac{T+1}{2} - \frac{x}{2} \right)
$$
where $C = \frac{2^T}{\Wealth_{T-1} \cdot \pi \cdot T!} > 0$.
We calculate the derivatives of $h(x)$. We will use the polygamma functions $\psi^{(m)}(z) = \frac{d^{m+1}}{dz^{m+1}} \ln(\Gamma(z))$ to express them.
\begin{align*}
h'(x) & = - C \cdot \Gamma \left(\frac{T+1}{2} + \frac{x}{2} \right) \cdot \Gamma \left(\frac{T+1}{2} - \frac{x}{2} \right)
\left( \frac{1}{2} \psi^{(0)}\left(\frac{T+1}{2} + \frac{x}{2} \right) - \frac{1}{2} \psi^{(0)}\left(\frac{T+1}{2} - \frac{x}{2} \right) \right) \; ,
\\
h''(x) & = - C \cdot \Gamma \left(\frac{T+1}{2} + \frac{x}{2} \right) \cdot \Gamma \left(\frac{T+1}{2} - \frac{x}{2} \right) \cdot \\
& \left[
\left( \frac{1}{2} \psi^{(0)}\left(\frac{T+1}{2} + \frac{x}{2} \right) - \frac{1}{2} \psi^{(0)}\left(\frac{T+1}{2} - \frac{x}{2} \right) \right)^2
+
\left( \frac{1}{4} \psi^{(1)}\left(\frac{T+1}{2} + \frac{x}{2} \right) + \frac{1}{4} \psi^{(1)}\left(\frac{T+1}{2} - \frac{x}{2} \right) \right)
\right]
\; .
\end{align*}
To apply Lemma~\ref{lemma:extremes}, we need to verify that $c \cdot h''(x) \le h'(x)$, which is equivalent to
\begin{multline*}
x \left( \frac{1}{2} \psi^{(0)}\left(\frac{T+1}{2} + \frac{x}{2} \right) - \frac{1}{2} \psi^{(0)}\left(\frac{T+1}{2} - \frac{x}{2} \right) \right)^2
+
x \left( \frac{1}{4} \psi^{(1)}\left(\frac{T+1}{2} + \frac{x}{2} \right) + \frac{1}{4} \psi^{(1)}\left(\frac{T+1}{2} - \frac{x}{2} \right) \right)
\\ \ge
\frac{1}{2} \psi^{(0)}\left(\frac{T+1}{2} + \frac{x}{2} \right) - \frac{1}{2} \psi^{(0)}\left(\frac{T+1}{2} - \frac{x}{2} \right) \; .
\end{multline*}
Making substitution $x = 2z$ and $a = (T+1)/2$, we need to prove that if $0 \le z < a$, then
$$
z \left( \psi^{(0)}\left(a + z \right) - \psi^{(0)}\left( a - z \right) \right)^2
+
z \left( \psi^{(1)}\left(a + z \right) + \psi^{(1)}\left(a - z \right) \right)
\ge
\psi^{(0)}\left(a + z \right) - \psi^{(0)}\left(a - z \right) \; .
$$
